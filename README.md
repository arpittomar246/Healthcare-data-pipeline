...
Simple PySpark-based pipeline that ingests healthcare data, cleans it, produces curated reports, and writes outputs.

- Tech: Python, PySpark (local), local parquet (development), S3 & JDBC hooks ready.
- Local demo: `src/run_pipeline_local_mode.py` runs fully locally using CSVs in `local_data/raw`.

.\.venv311\Scripts\Activate.ps1
python eda.py --raw-dir local_data/raw --out-dir local_data/artifacts
python run_gui.py
streamlit run run_dashboard.py
python -m src.runnerfile --config utils/project.cfg --force-fresh --skip-anonymize --open-ui